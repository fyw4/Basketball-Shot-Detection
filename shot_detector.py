...
class Shot_Detector:
    ...
    def update_score(self) -> None:
        ...
        for ballKey, hoopKey in deepcopy(self.down_ball):
            ...
            hoop = self.hoops[hoopKey] # 获取当前检测到的篮筐位置
            ball = self.balls[ballKey] # 获取当前检测到的球位置
            ...
            if x1_rim < x_pred < x2_rim: # 如果球在 hoop筐中心高度下的 x 坐标在篮筐的左右边界之间
                self.makes += 1
                self.goal_frames.append(self.frame_count) # 将进球的帧序号添加到 goal_frames 列表中
    ...
    def save_goal_clip(self, goal_frame, num_frames_before = 120, num_frames_after = 120):
        # 回溯到进球前120帧开始录制
        start_frame = max(0, goal_frame - num_frames_before) #确保起始帧不小于0
